...
count(which(!is.na(FM_prop_locid_change$F_prop_2318)))
...
r1_mprop <- lmer(M_prop ~ study_id + (1|S_HOOD), data = FM_prop_locid)
summary(r1_mprop)
...
```{r}
ggplot(FM_prop_locid_change, aes(x = F_prop_2018, y = F_prop_2023, colour = S_HOOD)) +
  geom_point() +
...
ggplot(FM_prop_locid_change, aes(x = M_prop_2018, y = M_prop_2023, colour = S_HOOD)) +
  geom_point() +
  geom_abline(slope = 1, linetype="dotted") +
  coord_cartesian(xlim = c(0.2, 1), ylim = c(0.2,1))
```
